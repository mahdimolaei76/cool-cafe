...
export default function OrderManagement() {
  ...
  const [takeawayFilter, setTakeawayFilter] = useState<'' | 'dine-in' | 'takeaway'>('');
  ...
  const filtered = useMemo(() => {
    ...
    if (takeawayFilter === 'takeaway') r = r.filter(o => o.isTakeaway);
    if (takeawayFilter === 'dine-in') r = r.filter(o => !o.isTakeaway);
    return r;
  }, [orders, search, statusFilter, takeawayFilter]);

  useEffect(() => { setPage(1); }, [search, statusFilter, takeawayFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <Input type="text" placeholder="جستجو با شماره سفارش، نام یا تلفن..." value={search} onChange={e => setSearch(e.target.value)} icon={<Search className="w-5 h-5" />} className="flex-1 min-w-64" />
        <select value={takeawayFilter} onChange={e => setTakeawayFilter(e.target.value as any)}
          className="px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-zinc-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
      ...
      <Modal open={!!selectedOrder} onClose={() => setSelectedOrderId(null)}
        title={`سفارش ${selectedOrder?.orderNumber ?? ''}`} size="lg" fixedHeight>
        {selectedOrder && (
          <OrderDetailModal
            order={selectedOrder}
