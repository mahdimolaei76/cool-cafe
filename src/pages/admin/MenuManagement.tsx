...
import { fetchDefaultMenuImages } from '@/lib/api';
...
export default function MenuManagement() {
  ...
  const [presetImages, setPresetImages] = useState<{ value: string; label: string }[]>([]);

  const [form, setForm] = useState({
    name: '', description: '', price: '', priceType: 'fixed' as 'fixed' | 'variable', priceLabel: '', categoryId: '', image: '', isAvailable: true, isFeatured: false,
  });

  // بارگذاری عکس‌های پیش‌فرض از پوشه public/images/defaultMenuImages
  useEffect(() => {
    fetchDefaultMenuImages().then(setPresetImages);
  }, []);
  ...
  const openCreate = () => {
    setEditingItem(null);
    setForm({ name: '', description: '', price: '', priceType: 'fixed', priceLabel: '', categoryId: categories[0]?.id || '', image: presetImages[0]?.value || '', isAvailable: true, isFeatured: false });
    setModalOpen(true);
  };
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1 space-y-3">
                ...
                {presetImages.length === 0 ? (
                  <p className="text-xs text-zinc-400 italic">در حال بارگذاری تصاویر...</p>
                ) : (
                  <div className="flex gap-2 flex-wrap max-h-40 overflow-y-auto pr-1">
                    {presetImages.map(img => (
                      <div key={img.value} className="relative group/img">
                        <button
